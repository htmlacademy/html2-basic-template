...
│   ├── less                      # Папка для препроцессорных файлов les
│   │   └── blocks/               # Стили БЭМ-блоков
│   │       └── header.less       # Стили для конкретного БЭМ-блока
│   │   └── global                # Файл для подключения стилей библиотек из папки vendor
│   │       ├── fonts.less        # Подключение шрифтов к проекту
│   │       ├── global.less       # Глобальные стили, которые каса.тся всего проекта
│   │       └── variables.less    # Переменные для всего проекта
...
## Работа со стилями
Все стили находится в папке `source/less`.
...
Все БЭМ-блоки и остальные препроцессорные файлы подключайте в `source/less/styles.less`.
